...
export class InteractiveMode implements InteractiveModeContext {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		// Brand-new sessions optionally start in the user's configured default mode.
		// "Brand-new" means the resolved branch carries no conversation context
		// (buildSessionContext().messages — covers messages, custom messages, branch
		// summaries, and compaction summaries) and the user set no explicit
		// `mode_change` (which #reconcileModeFromSession just restored). SDK startup
		// metadata and extension `custom` state entries are ignored. This way
		// `omp --continue` (or auto-resume) that finds no recent session and creates a
		// fresh one still honors the default, while a session with restored context or
		// an explicit mode keeps its reconciled mode. Scoped to launch (not the switch
		// reconciler above) so /new and mode handoffs never reactivate a startup
		// default. Plan keeps precedence when both defaults are enabled.
		...
		} else if (isFreshSession && this.session.settings.get("vibe.defaultOnStartup")) {
