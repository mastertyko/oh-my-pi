...
async function startServer(): Promise<string> {
	...
	server.on("stream", (stream: http2.ServerHttp2Stream, headers: http2.IncomingHttpHeaders) => {
		// Keep the request body drained; Run is bidirectional so the client
		// never ends its half, and the server must not wait for that.
		stream.on("data", () => {});
		...
		if (scenario.kind === "hang-after-turn") {
			// Leave the response open so clients must not treat turnEnded as done.
			return;
		}
...
async function stopServer(): Promise<void> {
	...
	await new Promise<void>(resolve => {
		...
		// Hard-stop stuck listeners so afterEach cannot hang the suite.
		setTimeout(() => {
			try {
				closing.close();
			} catch {
...
describe("Cursor terminal lifecycle after turnEnded", () => {
	...
	it("aborts without emitting done when the signal fires", async () => {
		...
		const streamPromise = collectStream(makeModel(baseUrl), { signal: controller.signal });
		// Allow the hang scenario to reach turnEnded, then abort so we don't wait unbounded.
		await Bun.sleep(50);
		controller.abort();
		const { eventTypes, result } = await streamPromise;
		expect(eventTypes[0]).toBe("start");
		expect(eventTypes.at(-1)).toBe("error");
		expect(eventTypes).not.toContain("done");
